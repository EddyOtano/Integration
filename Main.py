...
#List of supported schools that will be paired with other lists with admissions
#info.
...
def AskExtras():
    Extras = int(input("How many MEANINGFUL extracurriculars have you taken?\
 Meaningful meaning an impactful club, head or key member of a club, top player\
 in sports team, etc.\nAlso, add 1 to this number for every 50 hours of\
 volunteering done.\
 Type in numerical form as a whole number.\n"))
    return(Extras)
...
while running:
    ...
    print("Hello, this program will attempt to calculate the chances of you being \
 admitted into an University from a predesignated list of choices.")
    print("Co",end = "")
    print ("ntinue?")
    print ("Type","yes","if","you","wish","to","continue.",sep = " ")

    if str(input()).upper() == "YES": #User wants to continue
        print("Please type the name of one of the Universities listed below that\
 you want your chance of admission to be calculated for\
, exactly as seen in the list.")
        ...
        #% gets remainder
        ChanceBase = ((GPA/SelectSchool[0])/2 + (SAT/SelectSchool[1])/2 + (Extras/PretentiousnessLevel))/1.8
        ChanceEllipticIntegral = ChanceBase - ((1//(SelectSchool[2]))*.01)**(ChanceBase % PretentiousnessLevel)
